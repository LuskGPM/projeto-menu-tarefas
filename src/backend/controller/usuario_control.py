...
class UsuarioControler:
    ...
    def insert_user(self, dados: Dict) -> None | Exception:
    ...
    def validar_nickname(self, nickname: str) -> bool:
    ...
    def login(self, nickname: str, senha_do_front: str) -> None | ValueError:
        ...
        RedCache(cache_key=COMMON_KEYS['USUARIO_LOGADO'], cache_data=nickname).set_cache()
    
    def logout(self) -> str:
        delete_cache = RedCache(cache_key=COMMON_KEYS['USUARIO_LOGADO']).delete_cache()
        if delete_cache:
    ...
    def get_nickname_cache(self) -> str | None:
        cache_nickname = RedCache(COMMON_KEYS['USUARIO_LOGADO']).get_cache()
        if cache_nickname:
